...
def source_machines(project: ProjectModel, catalog: ComponentRegistry) -> frozenset[str]:
    """Chi la strada verso i terminali la fa **partire**: accumuli, puffer,
    scambiatori.

    Sono le tre parole del PO, e sono tre e non quattro: un **collettore** non
    e' una sorgente della distribuzione, e' il punto in cui la distribuzione si
    divide. Serve a due lettori — la gerarchia, che riconosce la strada
    secondaria, e le catene, che a quella strada concedono la curva di
    **D-144** — e sta qui perche' i due devono leggere lo stesso insieme.
    """
    definitions = {item.id: catalog.get(item.definition_id) for item in project.components}
