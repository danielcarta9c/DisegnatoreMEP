...
def test_the_same_rules_serve_a_medium_they_were_not_written_for() -> None:
    ...
    Il defangatore invece non esce: sul quel fluido il catalogo non ha nessun
    pezzo che separi i fanghi, e una regola senza niente da proporre non
    propone — non inventa e non fallisce."""
    registry = ComponentRegistry.from_directory(
        CATALOG, symbols=SymbolRegistry.from_directory(SYMBOLS)
    )
    found = evaluate(water_service(), registry, RuleRegistry.from_directory(RULES))
    by_rule = {item.rule_id: item for item in found}
    ...
    assert "dirt-separation-before-what-it-would-ruin" not in by_rule
