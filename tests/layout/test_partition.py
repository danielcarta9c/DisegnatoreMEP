...
def model(**overrides: object) -> ProjectModel:
    payload: dict[str, object] = {
        ...
        "networks": NETWORKS,
        "components": [
            {"id": item, "definition_id": "x"} for item in ("pump", "valve", "boiler")
        ],
        "connections": [
...
def test_declared_sheets_become_partitions_in_declared_order() -> None:
    ...
    assert set(parts[0].component_ids) == {"pump", "valve"}
    assert set(parts[1].component_ids) == {"boiler"}
...
def test_a_link_names_its_fluid_and_where_it_goes() -> None:
    ...
    assert item.connection_id == "s2"
    assert item.port.component_id in {"valve", "boiler"}


def test_a_run_keeps_every_accessory_even_when_a_boundary_cuts_it() -> None:
    project = two_sheets()
    trunks = build_trunks(project, frozenset({"valve"}))
    parts = partition_project(project, trunks)
    accessories = {cid for trunk in trunks for cid in trunk.inline_component_ids}
    placed = {cid for part in parts for cid in part.component_ids}
    assert accessories <= placed


def test_an_empty_sheet_is_refused() -> None:
    project = model(
        subsystems=[{"id": "plant", "name": "Centrale", "component_ids": ["pump", "valve", "boiler"]}],
        sheets=[
            {"id": "t1", "title": "Centrale", "subsystem_ids": ["plant"],
             "band_assignments": [{"subsystem_id": "plant", "band": "generation", "order": 0}]},
...
def test_a_component_in_no_subsystem_is_refused_when_sheets_are_declared() -> None:
    project = model(
        subsystems=[{"id": "plant", "name": "Centrale", "component_ids": ["pump", "valve"]}],
        sheets=[
            {"id": "t1", "title": "Centrale", "subsystem_ids": ["plant"],
             "band_assignments": [{"subsystem_id": "plant", "band": "generation", "order": 0}]},
        ],
    )
    with pytest.raises(LayoutError, match=r"belong to no sheet.*\['boiler'\]"):
        partition_project(project, build_trunks(project, frozenset({"valve"})))
