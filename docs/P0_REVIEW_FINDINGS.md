...
| ~~W8~~ | **Risolto dalla fase grafica (D-043).** La geometria vive nel manifesto del simbolo, `symbol_id` risolve contro una libreria reale, gli ancoraggi delle etichette esistono e l'area di rispetto è per lato. Testo originale: la geometria vive sulla definizione di componente, non sul simbolo, e `symbol_id` punta a una libreria inesistente. Mancano i punti di ancoraggio delle etichette; l'area di rispetto è uno scalare, quindi non puo' essere anisotropa | Primo giorno di P2 |
| ~~W9~~ | **Risolto dalla fase grafica (D-044).** `angle_deg` non esiste più: la direzione uscente è derivata dalla faccia, e la faccia è vincolata al lato su cui la porta si trova. Testo originale: nessun invariante lega `angle_deg` alla posizione della porta, e in tutte le fixture vale `0` anche per porte su lati opposti. Come dati di prova per l'instradamento sono inutilizzabili e vanno riscritti | Prima che P3 ne scriva centinaia nello stesso stile |
