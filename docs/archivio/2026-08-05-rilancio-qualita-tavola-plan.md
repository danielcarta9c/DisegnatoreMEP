...
- **Il collaudo** — un agente con contesto separato, che non ha visto nascere il lavoro —
  ...
  (`docs/standard/QUALITA_GRAFICA.md`), la regressione completa (test, lint, tipi, determinismo).
...
| WP5 — sviluppo eseguito | Sviluppatore WP5 | in attesa di collaudo | 2026-08-05 | Nove misure di qualità dentro il comando di disegno, con lo stesso vocabolario di esiti dei controlli di correttezza: pieghe per tratta, attraversamenti, sovrapposizione longitudinale (bloccante), distanze di rispetto, andate-ritorno (bloccante), richiami non a 45° (bloccante), riempimento del foglio, riempimento della tavola successiva (bloccante), fonte di ogni simbolo (bloccante). 27 prove su geometrie sintetiche. **La tavola oggi non esce**: il preflight blocca su richiami, andate-ritorno e distanze, ed è il comportamento voluto |
| Scoperte del preflight, non previste | Sviluppatore WP5 | — | 2026-08-05 | (1) La regola «le linee non sfiorano i simboli» era marcata *garantita dal motore* ed era falsa: tre violazioni vere a distanza zero, tratte che passano a filo di accessori appartenenti ad **altre** tratte. Difetto reale, mai visto prima, da chiudere (WP3c). (2) Tre prove di accettazione ora falliscono perché pretendono che il disegno esca: è il preflight che fa il suo mestiere su una catena in cui i richiami a 45° non sono ancora arrivati. Nessuna prova è stata ammorbidita per farle passare. (3) Carta del colpo d'occhio aggiornata dall'orchestratore: nove regole passano a *misurata*, due erano classificate male in partenza |
