...
| **A — l'indirizzo dei nodi (D-105)** | Orchestratore | in sviluppo | 2026-08-06 (sera) | **Deviazione registrata prima di eseguire:** lo sviluppatore separato assegnato al pacchetto e' stato interrotto dal limite di spesa dell'account prima di scrivere qualunque file; gli agenti di sviluppo non sono piu' disponibili in questa sessione. Sviluppa l'orchestratore, sui criteri gia' scritti in §3bis-A, e il pacchetto resta **da collaudare da contesto separato** finche' un collaudo indipendente non torna possibile: per D-083 senza quel verdetto non e' «fatto». |
